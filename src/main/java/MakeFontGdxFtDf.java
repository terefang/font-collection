...
public class MakeFontGdxFtDf
{
    ...
    public static void makeFont(File _ttf, int _size, int _xyres, File _outdir, String _prefix, int[][] _codesets)
    {
        ...
        for(int[] _charr : _codesets) for(int _char=_charr[0]; _char<(_charr[1]+1); _char++)
        {
            ...
            _pw.println(MessageFormat.format("char id={0,number,######0} x={1,number,######0} y={2,number,######0} width={3} height={4} xoffset=0 yoffset={5} xadvance={6} page={7} chnl=0 ",_char, _x>>2, _y>>2, _px.getWidth()>>2, _px.getHeight()>>2, _gl.getLeft()>>2, _gl.getTop()>>2, _page));
        ...
        _pw.println("info face=\"Sans\" size="+(_size>>2)+" bold=0 italic=0 charset=\"latin1\" unicode=0 stretchH=100 smooth=1 aa=1 padding=1,1,1,1 spacing=0,0");
        _pw.println("common lineHeight="+(_size>>2)+" base=0 scaleW="+(_xyres>>2)+" scaleH="+(_xyres>>2)+" pages="+(_page+1)+" packed=0");
    ...
    public static Pixmap getPixmap(FreeType.Glyph mainGlyph) {
        ...
        return mainBitmap.getPixmap(Pixmap.Format.RGBA8888, com.badlogic.gdx.graphics.Color.WHITE, 1f);
    }
}
