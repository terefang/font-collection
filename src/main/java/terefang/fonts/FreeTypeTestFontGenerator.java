...
public class FreeTypeTestFontGenerator
{
    ...
    public FreeTypeTestFontGenerator (FileHandle fontFile) {
        this(fontFile, 0);
    }
    ...
    public FreeTypeTestFontGenerator (FileHandle fontFile, int faceIndex) {
    ...
    private int getLoadingFlags (FreeTypeTestFontGenerator.FreeTypeFontParameter parameter) {
    ...
    private boolean checkForBitmapFont () {
        ...
        System.err.println(Integer.toHexString(faceFlags));
        if (((faceFlags & FreeType.FT_FACE_FLAG_FIXED_SIZES) == FreeType.FT_FACE_FLAG_FIXED_SIZES)
            ...
            if (loadChar(32))
            {
                ...
                System.err.println(Integer.toHexString(slot.getFormat()));
                if (slot.getFormat() == 1651078259)
                {
                    bitmapped = true;
                }
                else
                if (slot.getFormat() == 0x62697473) // 0x62697473 'bits'
                {
                    bitmapped = true;
                }
                else
                if (slot.getFormat() == 0x6F75746C) //0x6F75746C 'outl'
                {
                    bitmapped = false;
                }
            }
    ...
    public BitmapFont generateFont (FreeTypeTestFontGenerator.FreeTypeFontParameter parameter) {
        return generateFont(parameter, new FreeTypeTestFontGenerator.FreeTypeBitmapFontData());
    }
    ...
    public BitmapFont generateFont (FreeTypeTestFontGenerator.FreeTypeFontParameter parameter, FreeTypeTestFontGenerator.FreeTypeBitmapFontData data) {
     ...
     * eg for a RTL font.
     * @see BitmapFont#BitmapFont(BitmapFont.BitmapFontData, Array, boolean) */
     ...
     * then {@link FreeTypeTestFontGenerator.GlyphAndBitmap#bitmap} will be null. */
    public @Null
    FreeTypeTestFontGenerator.GlyphAndBitmap generateGlyphAndBitmap (int c, int size, boolean flip) {
        ...
        FreeTypeTestFontGenerator.GlyphAndBitmap result = new FreeTypeTestFontGenerator.GlyphAndBitmap();
        result.glyph = glyph;
        result.bitmap = bitmap;
        return result;
    }

    /** Generates a new {@link BitmapFont.BitmapFontData} instance, expert usage only. Throws a GdxRuntimeException if something went wrong.
     * @param size the size in pixels */
    public FreeTypeTestFontGenerator.FreeTypeBitmapFontData generateData (int size) {
        FreeTypeTestFontGenerator.FreeTypeFontParameter parameter = new FreeTypeTestFontGenerator.FreeTypeFontParameter();
        parameter.size = size;
        return generateData(parameter);
    }

    public FreeTypeTestFontGenerator.FreeTypeBitmapFontData generateData (FreeTypeTestFontGenerator.FreeTypeFontParameter parameter) {
        return generateData(parameter, new FreeTypeTestFontGenerator.FreeTypeBitmapFontData());
    }
    ...
    /** Generates a new {@link BitmapFont.BitmapFontData} instance, expert usage only. Throws a GdxRuntimeException if something went wrong.
     * @param parameter configures how the font is generated */
    public FreeTypeTestFontGenerator.FreeTypeBitmapFontData generateData (FreeTypeTestFontGenerator.FreeTypeFontParameter parameter, FreeTypeTestFontGenerator.FreeTypeBitmapFontData data) {
    ...
    protected @Null
    BitmapFont.Glyph createGlyph (char c, FreeTypeTestFontGenerator.FreeTypeBitmapFontData data, FreeTypeTestFontGenerator.FreeTypeFontParameter parameter, FreeType.Stroker stroker,
        ...
        try {
            ...
            mainGlyph.dispose();
            Gdx.app.log("FreeTypeTestFontGenerator", "Couldn't render char: " + c);
            return null;
        }
    ...
    /** {@link BitmapFont.BitmapFontData} used for fonts generated via the {@link FreeTypeTestFontGenerator}. The texture storing the glyphs is
     * held in memory, thus the {@link #getImagePaths()} and {@link #getFontFile()} methods will return null.
     * @author mzechner
     * @author Nathan Sweet */
    static public class FreeTypeBitmapFontData extends BitmapFont.BitmapFontData implements Disposable {
        public Array<TextureRegion> regions;

        // Fields for incremental glyph generation.
        FreeTypeTestFontGenerator generator;
        FreeTypeTestFontGenerator.FreeTypeFontParameter parameter;
            ...
            if (glyph == null && generator != null) {
                ...
                if (parameter.kerning) {
                    ...
                    for (int i = 0, n = glyphs.size; i < n; i++) {
                        BitmapFont.Glyph other = glyphs.get(i);
    ...
    /** Parameter container class that helps configure how {@link FreeTypeTestFontGenerator.FreeTypeBitmapFontData} and {@link BitmapFont} instances are
     * generated.
     *
     ...
     * a power-of-two sized texture, and the resulting {@link FreeTypeTestFontGenerator.FreeTypeBitmapFontData} will have a valid {@link TextureRegion} which
    ...
    public static class FreeTypeFontParameter {
        ...
        public boolean mono;
        /** Strength of hinting */
        public FreeTypeTestFontGenerator.Hinting hinting = FreeTypeTestFontGenerator.Hinting.AutoMedium;
         ...
         * @see FreeTypeTestFontGenerator.FreeTypeFontParameter */
         ...
         * FreeTypeTestFontGenerator must not be disposed until the font is no longer needed. The FreeTypeBitmapFontData must be
         ...
         * {@link FreeTypeTestFontGenerator#getMaxTextureSize()}. */
